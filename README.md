...
一个基于 Cloudflare Workers + D1 的轻量级密码管理器，支持管理员功能。
...
- **新增管理员功能**：用户管理、数据统计、全局密码查看

## 管理员功能

### 功能概述
- **仪表板**：系统统计（用户数、管理员数、密码总数、2FA启用率）
- **用户管理**：查看所有用户、查看用户详情、删除用户、设置/撤销管理员权限
- **密码总览**：查看所有用户的密码信息（不含密码明文）

### 如何设置管理员
1. 首次部署后，需要手动在数据库中将某个用户设置为管理员：
   ```sql
   UPDATE users SET is_admin = 1 WHERE email = 'admin@example.com';
   ```
2. 之后管理员可以在管理员面板中设置其他用户为管理员

### 访问管理员面板
- 管理员登录后，在用户界面右上角会看到 👑 按钮
- 点击进入管理员面板，可进行各项管理操作
- 可随时返回用户界面
## 安全与部署指南
